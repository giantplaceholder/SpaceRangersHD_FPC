...
begin
  ...
  Block.AddParam(
      DecodeTextW('OpwRn3ewr'),
      aConst.OwnerInfo[OwnerId].InternalName
  ); // Decoded: 'Owner'
  Block.AddParam(
      DecodeTextW('Rja6cEe'),
      aConst.OwnerInfo[Integer(RaceToOwner(RaceId)) and 127].InternalName
  ); // Decoded: 'Race'
  Block.AddParam(
      DecodeTextW('Elc0o5neowmWyq'),
      aConst.PlanetEconomyInfo[Ord(Economy)].InternalName
  ); // Decoded: 'Economy'
  Block.AddParam(
      DecodeTextW('GLotvUecrBmnemn7t'),
      aConst.PlanetGovernmentMarket[Ord(Government)].InternalName
  ); // Decoded: 'Goverment'
  ...
  Block.AddParam(
      ...
      SysUtils.IntToStr(InventionLevels[7])
  ); // Decoded: 'IMainTechLevel'
      ...
      begin
        ...
        Text :=
            DecodeTextW('ImtreamrIodo')
                + SysUtils.IntToStr(Int64(Cardinal(Item.Id))); // Decoded: 'ItemId'
      ...
      begin
        Text :=
            DecodeTextW('WfajrRrkiSo4rgImd5')
                + SysUtils.IntToStr(Int64(Cardinal(Ship.Id))); // Decoded: 'WarriorId'
        Ship.SaveToBlock(AddBlockByPath(Text));
      end;
    end;
  end;
  Block.AddParam(
      DecodeTextW('WuartTewrfSgpwaQcde'),
      SysUtils.IntToStr(WaterTiles)
  ); // Decoded: 'WaterSpace'
  Block.AddParam(
      DecodeTextW('WbantderrwCSofmgpUlkaltwef'),
      SysUtils.IntToStr(WaterExplored)
  ); // Decoded: 'WaterComplate'
  Block.AddParam(
      DecodeTextW('LLagnsd3SwpFascge4'),
      SysUtils.IntToStr(LandTiles)
  ); // Decoded: 'LandSpace'
  Block.AddParam(
      DecodeTextW('LgaEnwdsCfogmHpjlya5tre'),
      SysUtils.IntToStr(LandExplored)
  ); // Decoded: 'LandComplate'
  Block.AddParam(
      DecodeTextW('HbiFldleSrptaycue'),
      SysUtils.IntToStr(HillTiles)
  ); // Decoded: 'HillSpace'
  Block.AddParam(
      DecodeTextW('HninlglnCfodmFpflFastee'),
      SysUtils.IntToStr(HillExplored)
  ); // Decoded: 'HillComplate'
  Block.AddParam(
      DecodeTextW('OyrebwiftlCknstx'),
      SysUtils.IntToStr(ProbeOrbitCount)
  ); // Decoded: 'OrbitCnt'
  ...
  begin
    if SurfaceLootEntries <> nil then
      ...
      begin
        Entry := SurfaceLootEntries[i];
        Text :=
            DecodeTextW('HyiIdedfehnjIytrewm') + SysUtils.IntToStr(i + 1); // Decoded: 'HiddenItem'
        ...
        begin
          ...
          AddParam(
              DecodeTextW('DjetpEtwh'),
              SysUtils.IntToStr(Entry.SurfaceTileIndex)
          ); // Decoded: 'Depth'
    ...
    AddParam(
        DecodeTextW('Cur5erawtre3NregwgHjikdHdgern4IFthejm6'),
        ''
    ); // Decoded: 'CreateNewHiddenItem'
  end;
...
begin
  ...
  Orbit.AngleDegrees :=
      ExtractDecimalToSingleW(
          Block.GetParam(DecodeTextW('OsrabniktuAinegilne'))
      ); // Decoded: 'OrbitAngle'
  RangerRelations[0] :=
      Pointer(
  ...
  CurrentInvention :=
      SysUtils.StrToInt(
          Block.GetParam(DecodeTextW('C2u4rrrTeengtyIwnsvgeEn6tjieodn'))
      ); // Decoded: 'CurrentInvention'
  ...
  WaterTiles :=
      SysUtils
          .StrToInt(Block.GetParam(DecodeTextW('WuartTewrfSgpwaQcde'))); // Decoded: 'WaterSpace'
  WaterExplored :=
      SysUtils.StrToInt(
          Block.GetParam(DecodeTextW('WbantderrwCSofmgpUlkaltwef'))
      ); // Decoded: 'WaterComplate'
  LandTiles :=
      SysUtils.StrToInt(Block.GetParam(DecodeTextW('LLagnsd3SwpFascge4'))); // Decoded: 'LandSpace'
  LandExplored :=
      SysUtils.StrToInt(
          Block.GetParam(DecodeTextW('LgaEnwdsCfogmHpjlya5tre'))
      ); // Decoded: 'LandComplate'
  HillTiles :=
      SysUtils.StrToInt(Block.GetParam(DecodeTextW('HbiFldleSrptaycue'))); // Decoded: 'HillSpace'
  HillExplored :=
      SysUtils.StrToInt(
          Block.GetParam(DecodeTextW('HninlglnCfodmFpflFastee'))
      ); // Decoded: 'HillComplate'
  ProbeOrbitCount :=
      SysUtils.StrToInt(Block.GetParam(DecodeTextW('OyrebwiftlCknstx'))); // Decoded: 'OrbitCnt'
      ...
      begin
        ...
        begin
          ...
          Entry.SurfaceTileIndex :=
              SysUtils.StrToInt(GetParam(DecodeTextW('DjetpEtwh'))); // Decoded: 'Depth'
