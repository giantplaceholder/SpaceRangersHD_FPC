...
begin
  ...
  if not GetPlayer.InNormalSpace then
    Galaxy.PrimeIntegrityChecksum(134);
end;
...
begin
  if GetPlayer <> nil then
    if not GetPlayer.InNormalSpace then
      Galaxy.CheckIntegrityChecksum(135);
  if GetPlayer <> nil then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if not GetPlayer.InNormalSpace then
            Galaxy.CheckIntegrityChecksum(136);
          if GetPlayer.InNormalSpace then
          begin
            if DraggedGoodsIndex < 10 then
            begin
          ...
          if not GetPlayer.InNormalSpace then
            Galaxy.PrimeIntegrityChecksum(137);
          if GetPlayer.IsOnPlanet then
...
begin
  ...
  begin
    ...
    if not GetPlayer.InNormalSpace then
      Galaxy.CheckIntegrityChecksum(138);
    ...
    begin
      ...
      if not GetPlayer.InNormalSpace then
        Galaxy.PrimeIntegrityChecksum(139);
      RefreshGoodsDisplay;
      MainPanel.RebuildMessageButtons(False);
    end;
  end;
...
begin
  ...
  if not GetPlayer.InNormalSpace then
    Galaxy.CheckIntegrityChecksum(142);
  while True do
  begin
  ...
  if not GetPlayer.InNormalSpace then
    Galaxy.PrimeIntegrityChecksum(143);
  AmbientSound.SetVolume(1);
  if GetPlayer.IsOnPlanet then
    if GetPlayer.CurrentPlanet.GetRelationLevelToShip(GetPlayer) = rlHostile then
    begin
